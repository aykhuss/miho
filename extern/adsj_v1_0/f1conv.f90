    ...
    !-------------------------------------------------------------------
	  ...
	  if (dabs(x-1).lt.zero .and. dabs(y-1).lt.zero) then 
		  ...
		  f1conv = cgamma(cc)*cgamma(cc-ca-cbp)*f21(ca,cb,cc-cbp,cx)*(cgammar(cc-ca)*cgammar(cc-cbp))
		  return
	  else if(dabs(x-1).lt.zero) then
		  f1conv = cgamma(cc)*cgamma(cc-ca-cb)*f21(ca,cbp,cc-cb,cy)*(cgammar(cc-ca)*cgammar(cc-cb))
		  return
