    ...
    complex*16 function g2(a1,a2,b1,b2,x,y)
	  ...
	  g2 =(1+cx)**(-a1)*(1+cy)**(-a2)*f2(1-b1-b2,a1,a2,1-b1,1-b2,cx/(cx+1),cy/(cy+1))
	  if (debug) write(*,*) g2
    ...
    if(flag.eq.1)	then
  		  ...
  		  s=(1-cx)**(-a)*f2s(a,c1-b1,b2,c1,c2,cx/(cx-1),cy/(1-cx))
	  else if(flag.eq.22)	then  
  		  if (debug) write(*,*) "Transformation 2y:",cx/(1-cy)," ",cx/(1-cy) 
  		  s=(1-cy)**(-a)*f2s(a,b1,c2-b2,c1,c2,cx/(1-cy),cy/(cy-1))
	  else if(flag.eq.3)	then
  		  if (debug) write(*,*) "Transformation 3: ",cx/(cx+cy-1)," ",cy/(cx+cy-1) 
  		  s=(1-cx-cy)**(-a)*f2s(a,c1-b1,c2-b2,c1,c2,cx/(cx+cy-1),cy/(cx+cy-1))	 
	  else  
  ...
  !	if (debug) write(*,*) i
    ...
    !-------------------------------------------------------------------
	  ...
	  cx = x*(1d0,0d0)
	  cy = y*(1d0,0d0)
	  f2term = coef2(a,b1,b2,c1,c2,n)*(x*y)**n*f21(a+n,b1+n,c1+n,cx)*f21(a+n,b2+n,c2+n,cy)
	  return
	  end
    !-------------------------------------------------------------------
