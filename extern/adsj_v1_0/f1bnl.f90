    ...
    !
	    if (x.eq.1 .and. y.eq.1) then 
		    ...
		    f1bnl = cgamma(cc)*cgamma(cc-ca-cbp)*f21(ca,cb,cc-cbp,cx)*(cgammar(cc-ca)*cgammar(cc-cbp))
		    return
	    else if(abs(x-1).lt.zero) then
		    f1bnl = cgamma(cc)*cgamma(cc-ca-cb)*f21(ca,cbp,cc-cb,cy)*(cgammar(cc-ca)*cgammar(cc-cb))
		    return
