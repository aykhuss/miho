...
double Scale1DGeometricModel::sigma(int order) const {
...
double Scale1DGeometricModel::pdf(const double& val) const {
...
double Scale1DGeometricModel::pdf_trapezoid(const double& val) const {
  // double log_min = log(_scale_models.begin()->first.fac_mu);
  // double log_max = log(_scale_models.rbegin()->first.fac_mu);
  // std::cout << "# Scale1DGeometricModel: marginalising log(mu) [" << log_min
  //           << ", " << log_max << "]\n";
...
double Scale1DGeometricModel::pdf_gauss_legendre(const double& val) const {
  if (_scale_models.size() != 3)
    throw std::runtime_error(
        "Scale1DGeometricModel::pdf_gauss_legendre: incompatible # of modles");
  ...
  for (const auto& scl_gm : _scale_models) {
    if (is_approx(scl_gm.first.fac_mu, gauss_legendre_weights.at(i).first)) {
      ...
      throw std::runtime_error(
          "Scale1DGeometricModel::pdf_gauss_legendre: invalid scale value?!");
    }
    i++;
  }
  return result_num / result_den;
